...
const defaultSettings = {
    production: {
        ...
        ca_cert: fs.readFileSync(`./certs/ftfrejaeid_prod.ca`),
        display_name: '',
        policy: ''
    },
    testing: {
        endpoint: 'https://grpt.funktionstjanster.se:18898/grp/v2?wsdl',
        ca_cert: fs.readFileSync(`./certs/ftfrejaeid_test.ca`),
        display_name: 'test',
        policy: 'logtest020'
    }        
}
...
async function initRequest(self,service,method,data) {
    ...
    return {status: 'initialized', id: result.transactionId+"-"+result.orderRef, extra: {
        autstart_token: result.AutoStartToken,
        autostart_url: "frejaeid://bindUserToTransaction?transactionReference="+encodeURIComponent(result.AutoStartToken)
    }};
